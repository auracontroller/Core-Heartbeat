...
class Heartbeat {
    ...
    start() {
        ...
        this.pulseInterval = setInterval(() => {
            ...
            // Step 1: Strike the core to verify its baseline health
            const coreResponding = this.core.receivePing();
            ...
            // Step 2: Request the status bundle for the current module in the rotation
            const statusBundle = this.core.getCurrentModuleStatus();
            if (statusBundle) {
    ...
    triggerUserIntervention(moduleId, port, failureType) {
        ...
        rl.question('Would you like to attempt to restart the module? (yes/no): ', (answer) => {
            if (answer.toLowerCase() === 'yes' || answer.toLowerCase() === 'y') {
                rl.question('Check this box to roll back state by 5 seconds? (yes/no): ', (rollbackAnswer) => {
                    const useRollback = rollbackAnswer.toLowerCase() === 'yes' || rollbackAnswer.toLowerCase() === 'y';

                    rl.close();
                    this.executeRecovery(moduleId, port, useRollback);
                });
            } else {
                this.log(`User declined restart for Module ${moduleId}. Remaining in quarantine state.`);
                console.log(`Module ${moduleId} remains isolated. System running on surviving modules.`);
                rl.close();
                // Keep system running without this module; release heartbeat suspension
                this.isSuspended = false;
            }
        });
    }
    ...
    executeRecovery(moduleId, port, useRollback) {
        ...
        if (restartSuccessful) {
            ...
            this.moduleStates.set(moduleId, { missedPulses: 0 });
            this.isSuspended = false; // Release the metronome to resume normal pulsing
