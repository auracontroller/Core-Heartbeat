...
SOCKET connect_to_core(int& my_port_id) {
    ...
    if (connect(sock, (struct sockaddr *)&server_addr, sizeof(server_addr)) == SOCKET_ERROR) {
        cerr << "Heartbeat 1: Connection failed." << endl << flush;
    ...
    if (bytes_read > 0) {
        ...
        if (pos != string::npos) {
            ...
            if (end_pos != string::npos) {
                ...
                cout << "Heartbeat 1 connected. Assigned Dock ID: " << my_port_id << endl << flush;
            }
        }
    }
    return sock;
...
void receive_loop(SOCKET sock) {
    ...
    while (true) {
        int bytes_read = recv(sock, buffer, sizeof(buffer) - 1, 0);
        if (bytes_read <= 0) {
            cerr << "Heartbeat 1: CoreRelay dropped. Violently freeing RAM." << endl << flush;
            exit(0);
        }
        ...
        while ((pos = leftover.find('\n')) != string::npos) {
            ...
            if (from_pos != string::npos && payload_pos != string::npos) {
                ...
                if (payload == "SUSPEND_ALL" && system_state != SUSPENDED) {
                    cout << "Heartbeat 1: External SUSPEND caught. Locking lattice." << endl << flush;
                    system_state = SUSPENDED;
                }
                else if (payload == "RESUME") {
                    cout << "Heartbeat 1: RESUME caught. Thawing lattice." << endl << flush;
                    system_state = NORMAL;
                    strikes = 0;
...
int main() {
    ...
    cout << "Heartbeat 1: Booting up. Waiting 2 seconds for lattice to settle..." << endl << flush;
...

    {
        ...
        should_speak = true;
        last_fuse_reset = chrono::steady_clock::now();
        cout << "Heartbeat 1: Ignition sequence active. Firing first pulse." << endl << flush;
    }
    ...
    while (true) {
        ...
        if (system_state == NORMAL) {
            ...
            if (elapsed > 1000) { 
                ...
                if (strikes == 1) {
                    cout << "Heartbeat 1: Warning. Echo dropped. Strike 1. Firing secondary pulse." << endl << flush;
                    should_speak = true;
                }
                else if (strikes >= 2) {
                    cout << "Heartbeat 1: Critical Silence. Strike 2 threshold breached. FIRING SUSPEND_ALL." << endl << flush;
                    system_state = SUSPENDED;
                    should_speak = false;
                }
            }
